...
source('code/monthly cycle length/generate_input_parameters.R')
...
if(is.infinite(as.numeric(final_age))) {starting_age <- 90 }else{
  if (initial_age == "0-") {starting_age <- 50}else{starting_age <- ceiling((as.numeric(final_age)+as.numeric(initial_age))/2)}
}
# Specify the gender
gender <- "female"

# Define global scenario parameters
if (initial_age == "0-") {n_cycles <- 50*12}else{
  n_cycles <- (100- as.numeric(initial_age))*12}
