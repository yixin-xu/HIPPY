...
generate_transition_matrices <- function(input_parameters, 
                                         ...
                                         gender = gender,
                                         n_cycles=n_cycles, sensitivity = NULL) {
  ...
  log_rate_primary <- rnorm(n_samples, mean = as.numeric(primary_mortality_90d_raw[primary_mortality_row_index,6]), 
                            sd = as.numeric(primary_mortality_90d_raw[primary_mortality_row_index,7])) 
  rate_primary <- exp(log_rate_primary) / 1000 
  first_mortality <- 1 - exp(-rate_primary)
  
 
  log_rate_revision <- rnorm(n_samples, mean = as.numeric(revision_mortality_90d_raw[revision_mortality_row_index,5]), 
                            sd = as.numeric(revision_mortality_90d_raw[revision_mortality_row_index,6])) 
  rate_revision <- exp(log_rate_revision) / 1000 
  second_mortality <- 1 - exp(-rate_revision)
  ...
  for(i_cycle in 1:n_cycles) {
    for(treatment_name in treatment_names) {
      transition_matrices[i_cycle, treatment_name, , "State Post THR <2 years", "State Early revision"] <- 
        (1 - exp(-exp(input_parameters[, paste0("log_rate_1st_revision_<2", treatment_name)])))*(1-first_mortality)
      transition_matrices[i_cycle, treatment_name, , "State Post THR >=2 years < 10 years", "State middle revision"] <- 
        (1 - exp(-exp(input_parameters[, paste0("log_rate_1st_revision_2-10", treatment_name)])))*(1-first_mortality)
      transition_matrices[i_cycle, treatment_name, , "State Post THR >=10 years", "State late revision"] <- 
     ...
     # }
      ...
      transition_matrices[ i_cycle, treatment_name, , "State Post THR <2 years", "State Death"] = rep(1-exp(-as.numeric(lifetime[starting_age + i_cycle - 1, gender])), n_samples) + 
        (1 - exp(-exp(input_parameters[, paste0("log_rate_1st_revision_<2", treatment_name)])))*first_mortality
      transition_matrices[ i_cycle, treatment_name, , "State Post THR >=2 years < 10 years", "State Death"] = rep(1-exp(-as.numeric(lifetime[starting_age + i_cycle - 1, gender])), n_samples) + 
        (1 - exp(-exp(input_parameters[, paste0("log_rate_1st_revision_2-10", treatment_name)])))*first_mortality
      transition_matrices[ i_cycle, treatment_name, , "State Post THR >=10 years", "State Death"] = rep(1-exp(-as.numeric(lifetime[starting_age + i_cycle - 1, gender])), n_samples) + 
      ...
      transition_matrices[ i_cycle, treatment_name, , "State Post THR <2 years", "State Post THR >=2 years < 10 years"] = (1-transition_matrices[ i_cycle, treatment_name, , "State Post THR <2 years", "State Early revision"]-
                                                                                                                             transition_matrices[i_cycle, treatment_name, , "State Post THR <2 years", "State Death"])/2
      
      
      
      transition_matrices[i_cycle, treatment_name, , "State Post THR >=2 years < 10 years", "State Post THR >=10 years"] = (1-transition_matrices[ i_cycle, treatment_name, , "State Post THR >=2 years < 10 years", "State middle revision"]-
                                                                                                                              transition_matrices[ i_cycle, treatment_name, , "State Post THR >=2 years < 10 years", "State Death"])/9
